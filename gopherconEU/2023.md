...
This, together with the efforts that are made by the Go team to increase the security levels in Go, were the highlights of the talk from Julie Qiu, the manager of the security team in Go, that explained about the concept of vulnerability and its relationship with attacks on the supply chain (with a nice cake analogy 😄🍰).

For this reason it was important to hear about idea behind the vulnerability database maintained by the go team and its application in the newly introduced `vulncheck` tool of the go toolchain and in the package documentation in <pkg.go.dev>.

On this page, in the context of a broader talk on the state of Go today, Cameron Balahan shared with us the importance of security to be able to make Go a platform that is production ready: reliable, effective and secure.

## Going beyond the next level of Go

This section is the trickiest to describe and I'll make my best to convey this feeling with these examples.

The first example I think of is the workshop of Bill Kennedy about Ultimate Services in Kubernetes where he shared several tips and trick to both create an application and deploy it in Kubernetes and to take well known Go concepts and take them to the next level.

An example of this:
accept structs before interfaces
http.handler with context and returning error

Another example I think of is the talk done by Julien Cretel about some tips and tricks to create functional options to help create better libraries as functional options are something that are somehow undervalued and can make a useful way 

those are some ideas that are difficult to learn without exchanging with these people or without much more experience on Go.
